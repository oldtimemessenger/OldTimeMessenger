...
- Authentication is Supabase Auth on the mobile client; the API verifies Supabase JWTs (or legacy opaque session tokens) and maps them to local Old Time users via `supabase_uid`.
...
- Stabilization comes before any architecture migration: do not perform the proposed core-data migration, merge the `old_time` schema into `public`, or change integer IDs to UUIDs without explicit approval.
