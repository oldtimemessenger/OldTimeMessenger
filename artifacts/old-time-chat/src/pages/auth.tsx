...
export default function AuthPage() {
  ...
  const submitOtp = (event: FormEvent) => { event.preventDefault(); setError(''); verifyOtp.mutate({ data: { phone, otp, challengeId } }, { onSuccess: (user: AuthenticatedUser) => { localStorage.setItem('old-time-user', JSON.stringify(user)); setLocation('/chats'); }, onError: (verifyError) => setError(verifyError instanceof Error ? verifyError.message : 'That code is invalid or has expired. Try again.') }); };
