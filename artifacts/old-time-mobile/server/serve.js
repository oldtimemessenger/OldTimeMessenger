...
const landingPageTemplate = require('./templates/landing-page');
...
function serveStaticFile(urlPath, res) {
  ...
  const boundaryPath = path.relative(STATIC_ROOT, filePath);

  if (boundaryPath.startsWith('..') || path.isAbsolute(boundaryPath)) {
    res.writeHead(403);
    res.end('Forbidden');
    return;
  }
  ...
  const content = fs.readFileSync(filePath); // nosemgrep: javascript.express.file.fs-express.fs-express
