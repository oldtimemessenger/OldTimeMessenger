...
function randomNonce(bytes = 32) {
  ...
  for (const byte of array) value += alphabet[byte % alphabet.length];
  return value;
}

export async function signInWithAppleNative(): Promise<'completed'> {
  assertSupabaseConfigured();
  if (Platform.OS !== 'ios') {
    throw new Error('Native Apple sign-in is only available on iPhone.');
  }
  ...
  if (fullName || credential.email) {
    ...
    await supabase.auth.updateUser({ data: metadata }).catch(() => undefined);
  }
  return 'completed';
}
...
export function getSafeAuthError(error: unknown, fallback = 'Please try again.') {
  ...
  if (message.includes('err_canceled') || message.includes('err_request_canceled') || message.includes('cancel')) return 'Sign-in was cancelled.';
  if (message.includes('identity token')) return 'Apple sign-in did not finish. Please try again.';
