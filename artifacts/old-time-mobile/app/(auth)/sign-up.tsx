...
export default function SignUpScreen() {
  ...
  const sendCode = async () => {
    ...
    setLoading(true);
    try {
      const { error } = await sendEmailCode(email, true);
  ...
  const resendCode = async () => {
    ...
    setLoading(true);
    try {
      const { error } = await sendEmailCode(emailAddress, true);
      if (error) throw error;
      setCode('');
      setSecondsRemaining(60);
    } catch (error) {
    ...
    >
      ...
      {codeSent ? (
        <>
          ...
          <Text style={[styles.eyebrow, { color: colors.action }]}>Create your account</Text>
          ...
          >
            <Text style={[styles.primaryButtonText, { color: colors.primaryForeground }]}>{loading ? 'Checking…' : 'Create account'}</Text>
        ...
        <>
          <Text style={[styles.eyebrow, { color: colors.action }]}>Join Old Time</Text>
          <Text style={[styles.title, { color: colors.foreground }]}>Create your account</Text>
          <Text style={[styles.body, { color: colors.mutedForeground }]}>Use your email to get a six-digit code. No password and no confirmation link.</Text>
          ...
          <Link href={'/(auth)/sign-in' as never} asChild>
            <Pressable style={styles.createLink}>
              <Text style={[styles.createLinkText, { color: colors.foreground }]}>Already have an account? <Text style={{ color: colors.action }}>Sign in</Text></Text>
            </Pressable>
          </Link>
          {hasOAuthProviders ? (
            <>
